...
class ApiService {
  ...
  Future<List<Device>> getDevices() async {
    ...
    // In production, this would make HTTP request to Serverpod
  ...
  // Production HTTP methods (commented for reference)
  /*
  Future<List<Device>> getDevices() async {
  ...
  Future<ButlerExplanation> explainAlert(int alertId) async {
    final response = await http.post(
      Uri.parse('$baseUrl/butler/explain'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode({'alertId': alertId}),
    );
    if (response.statusCode == 200) {
      return ButlerExplanation.fromJson(json.decode(response.body));
    }
